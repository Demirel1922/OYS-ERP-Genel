...
export function SalesOrderNew() {
  ...
  const onSubmit = useCallback(async (data: SalesOrderFormData) => {
    ...
    const orderNo = generateAuto ? undefined : customOrderNo;
    const id = await createOrder(order, orderNo);
    if (id) navigate('/module/4/siparis');
  }, [buildOrderPayload, createOrder, customOrderNo, generateAuto, navigate, confirmedLines]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={customOrderNo}
                    onChange={(e) => {
                      setCustomOrderNo(e.target.value);
                      setGenerateAuto(e.target.value === '');
                    }}
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => {
                        setGenerateAuto(e.target.checked);
                        if (e.target.checked) setCustomOrderNo('');
                      }}
                    />
                    Otomatik üret (SIP-YYYYMMDD-XXX)
                  </label>
                </div>
                ...
                {fields.map((field, index) => {
                  ...
                  return (
                    <Card key={field.id} className={`p-4 ${isConfirmed ? 'border-green-300 bg-green-50/30' : ''}`}>
                      {isConfirmed && (
                        <div className="mb-2 flex items-center gap-1 text-green-700 text-xs font-medium">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-12 gap-3">
                        ...
                        {/* Butonlar */}
                        <div className="col-span-2 md:col-span-1 flex items-end gap-1">
                          {!isConfirmed ? (
                            <>
                              <Button
                                type="button"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xl font-bold text-primary">
                      {formatMoney2(totals.total_amount, 'TRY')}
                    </div>
                  </div>
                </div>
              </div>
